...
class S3DirectoryReader(BaseReader):
    ...
    def __init__(
        ...
        if input_files:
            ...
            for path in input_files:
                input_file = S3Path(path)
                self.input_files.append(input_file)
        elif input_dir:
            self.input_dir = S3Path(input_dir)
            self.exclude = exclude
            self.input_files = self._add_files(self.input_dir)
            print('the input files in S3Path obj are')
            ...
            for excluded_pattern in self.exclude:
                if self.recursive:
                    ...
                    for file in input_dir.rglob(excluded_pattern):
                        rejected_files.add(S3Path(file))
                else:
                    # Non-recursive glob
                    for file in input_dir.glob(excluded_pattern):
                        rejected_files.add(S3Path(file))

        file_refs: Generator[S3Path, None, None]
        if self.recursive:
            file_refs = S3Path(input_dir).rglob("*")
        else:
            file_refs = S3Path(input_dir).glob("*")
